def topk_recall_glove_embedding(click_all, dict_label, k=100, dim=88, epochs=30,learning_rate=0.5):
    ...
    glove.fit(corpus_model.matrix, epochs=epochs, no_threads=2, verbose=True)
...
for phase in range(0, now_phase + 1):
    ...
    print('-------- 召回 -------------')
    topk_recall = topk_recall_glove_embedding(click_all=train,
