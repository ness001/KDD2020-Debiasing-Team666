...
train_merge_file = '..\\data\\train_merge_phase-%d.csv'
# recall_usercf_file = '..\\data\\recall_usercf_feat_0.csv'
...
for phase in range(8, 10):
    ...
    train_df = pd.read_csv(train_merge_file % phase,
                           header=None,
                           names=['user_id', 'item_id', 'itemcf_rank', 'itemcf_score', 'label', 'usercf_rank',
                                  'usercf_score', 'item_heat', 'user_heat'])
    print('\ntrain_df: ', train_df.shape)
    print(train_df.head())

    user_set=set(train_df['user_id'].values)
    print('len(user_set): ',len(user_set))

    user_itemlist_df=train_df.groupby('user_id')['item_id'].agg(list)
    ...
    for user in tqdm(user_set):
        ...
        # user_lists += [user]*len(item_list_1)
        txt_list_0 = []
        txt_list_1 = []
        img_list_0 = []
        img_list_1 = []
    ...
    df=pd.merge(df_new, train_df, how='left', on=['user_id','item_id'])
    print('df: ',df.shape)
    print(df.head())
    print(df.columns.values)
    df.to_csv('..\\data\\train_feat_phase-%d.csv' % phase, index=False, header=False)
