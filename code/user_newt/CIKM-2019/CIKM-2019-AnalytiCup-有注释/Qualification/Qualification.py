...
# df_log_selected['day_hour']在[5,20]之间
...
item_to_category_dict = dict()  # {'itemID': 'categoryID'}
for row in df_item.values:
    item_to_category_dict[row[0]] = row[1]


historicalDict = {}  # {'userID': {itemID1, itemID2, .....}}
...
his_cat_Dict = {}  # {'userID': {categoryID1, categoryID2, .....}}
...
def process(each_user):
    ...
    if len(df_tmp) > 0: # 用户存在购物历史
        ...
        itemListTmp = itemListTmp + list(item_sta_sorted.loc[:27,'itemID'])  #复购？？？
        

        
        if len(itemListTmp) < 50: #热度补全
