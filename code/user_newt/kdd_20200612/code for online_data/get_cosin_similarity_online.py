...
online_merge_file = '..\\online_data\\online_merge_phase-%d.csv'
...
for phase in range(7, 10):
    ...
    df_online = pd.read_csv(online_merge_file % phase,
                           header=None,
    ...
    user_set=set(df_online['user_id'].values)
    print('len(user_set): ',len(user_set))

    user_itemlist_df=df_online.groupby('user_id')['item_id'].agg(list)
    ...
    # df_online=df_online[~df_online['user_id'].isin(users_new)]
    df=pd.merge(df_new, df_online, how='left', on=['user_id','item_id'])
    print('df: ',df.shape)
    print(df.head())
    print(df.columns.values)
    df.to_csv('..\\online_data\\online_feat_phase-%d.csv' % phase, index=False, header=False)
