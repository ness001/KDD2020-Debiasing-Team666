...
for phase in range(start, end):
    ...
    neg_samples=shuffle(neg_samples,random_state=seed)[:ratio*len(pos_samples)]  #负样本的挑选对最后的结果有影响
    ...
    other_params = {'learning_rate': 0.1, 'n_estimators': 100, 'max_depth': 7, 'min_child_weight': 2, 'seed': 0,
                    'subsample': 0.8, 'colsample_bytree': 0.8, 'gamma': 0, 'reg_alpha': 0, 'reg_lambda': 1, 'scale_pos_weight':ratio//2}


    model = XGBClassifier(**other_params)
    # optimized_GBM = GridSearchCV(estimator=model, param_grid=cv_params, scoring='f1', cv=3, verbose=1, n_jobs=4)
    ...
    joblib.dump(model,  "..\\test_data\\xgboost_model_phase_%d.pkl" %phase)
