...
for c in range(7, now_phase + 1):
    ...
    user_sim_corr = usercf(underline_train)

    for index, row in tqdm(underline_val.iterrows()): #underline_val
    ...
    assert df.shape[1] == 5
    print(df.head())
    df.to_csv('..\\data\\train_usercf_last2_phase-%d.csv' %c, index=False, header=False)
