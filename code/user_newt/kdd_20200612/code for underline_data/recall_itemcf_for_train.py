...
for c in range(start, now_phase + 1):
    ...
    user_itemlist = underline_train.groupby('user_id')['item_id'].agg(list).reset_index()
    user_items_dict = dict(zip(user_itemlist['user_id'], user_itemlist['item_id']))
    with open('..\\data\\dict_user_items_phase-%d.txt' % c,'w') as fin:
        temp_str=str(user_items_dict)
        fin.write(temp_str)
    #
    item_userlist=underline_train.groupby('item_id')[['user_id']].agg(list).reset_index()
    dict_item_userlist=dict(zip(item_userlist['item_id'], item_userlist['user_id']))
    with open('..\\data\\dict_item_users_phase-%d.txt' % c,'w') as fin:
        temp_str=str(dict_item_userlist)
        fin.write(temp_str)
    #
    top500_click = list(underline_train['item_id'].value_counts().index[:500].values)  # 最热商品
    with open('..\\data\\list_topk_500_items_phase-%d' % c,'w') as f:
        temp_str=str(top500_click)
        f.write(temp_str)

    item_sim_corr = itemcf(click, user_items_dict, use_iif=False)

    # user_sim_corr = usercf(underline_train_val)

    for index, row in tqdm(underline_val.iterrows()):
    ...
    assert df.shape[1] == 5
    print(df.head())
    df.to_csv('..\\data\\train_itemcf_last2_phase-%d.csv' %c, index=False, header=False)
